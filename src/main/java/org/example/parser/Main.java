...
public class Main
{
    public static void main(String[] args) throws Exception
    {
        ...
        //list[0] = parser[0].parsePages(parser[0].getPageProductsListStore("samsung", 5000), 18000, 1);
        //list[0] = parser[1].parsePages(parser[1].getPageProductsListStore("флешка", 5000), 500, 1);
        //list[0] = parser[2].parsePages(parser[2].getPageProductsListStore("ps5", 5000), 1500 ,1);
        //list[0] = parser[3].parsePages(parser[3].getPageProductsListStore("наушники sony", 5000), 2000 ,1);
        ...
        CompletableFuture<List> com2 = CompletableFuture.supplyAsync(()->
        {
            try
            {
                list[1] = parser[1].parsePages(parser[1].getPageProductsListStore("флешка", 5000), 500, 1);
            } catch (IOException e)
            {
                e.printStackTrace();
            }
        ...
        CompletableFuture<List> com3 = CompletableFuture.supplyAsync(()->
        {
            try
            {
                list[2] = parser[2].parsePages(parser[2].getPageProductsListStore("ps5", 5000), 1500, 1);
            } catch (IOException e)
            {
                e.printStackTrace();
            }
        ...
        CompletableFuture<List> com4 = CompletableFuture.supplyAsync(()->
        {
            try
            {
                list[3] = parser[3].parsePages(parser[3].getPageProductsListStore("наушники sony", 5000), 2000, 1);
            } catch (IOException e)
            {
                e.printStackTrace();
            }
